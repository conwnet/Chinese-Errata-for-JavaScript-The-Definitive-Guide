...
- 位置：第 259 页，第三段文字（不算单独一段的正则表达式）。
- 错误内容：正则表达式不允许用双引号扩起来的内容中有单引号，反之亦然。
- 错误原因：我觉得这里语义不明，这里原文表述的是：这个正则表达式的作用是不允许使用双引号扩起来的内容有单引号，反之亦然。而我觉得中文版的意思有点儿模糊，不查原版还真没看明白。
- 应更正为：这个正则表达式的作用是不允许使用双引号扩起来的内容有单引号，反之亦然。

## 10.1.5 指定匹配位置

- 位置：第 260 页，第二段第三行的正则表达式。
- 错误内容：`/\s\Javas/`。
- 错误原因：这里想匹配前后都有空格的单词“Java”，笔误把正则写错了。
- 应更正为：`/\sJava\s/`。

## 10.1.5 指定匹配位置

- 位置：第 260 页，倒数第二段。
- 错误内容：用以指定接下来的字符都不必匹配。
- 错误原因：不是不必匹配，是不能匹配啊，原文用的是 must not。
- 应更正为：用以指定接下来的字符都不匹配。
